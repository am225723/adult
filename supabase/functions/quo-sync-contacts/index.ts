import { createClient } from "https://esm.sh/@supabase/supabase-js@2";

const QUO_API_KEY = Deno.env.get("QUO_API_KEY") ?? "";
const QUO_BASE = Deno.env.get("QUO_API_BASE") ?? "https://api.quo.com/v1";
const SUPABASE_URL = Deno.env.get("SUPABASE_URL")!;
const SUPABASE_SERVICE_ROLE_KEY = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;

const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);

const quoHeaders = () => ({
  Authorization: QUO_API_KEY,
  "Content-Type": "application/json",
});

function fetchWithTimeout(url: string, options: RequestInit, timeoutMs = 8000): Promise<Response> {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  return fetch(url, { ...options, signal: controller.signal }).finally(() =>
    clearTimeout(timer),
  );
}

async function syncContactsForWorkspace(workspaceId: string) {
  try {
    let after: string | undefined;
    let hasMore = true;
    let synced = 0;
    let errors = 0;

    while (hasMore) {
      const params = new URLSearchParams({ limit: "100" });
      if (after) params.append("after", after);

      const res = await fetchWithTimeout(`${QUO_BASE}/contacts?${params}`, {
        headers: quoHeaders(),
      });

      if (!res.ok) {
        console.error(`Failed to fetch contacts from Quo: ${res.statusText}`);
        errors++;
        break;
      }

      const data = await res.json();
      const contacts = (data.data || []) as Array<Record<string, unknown>>;

      for (const contact of contacts) {
        try {
          const defaultFields = contact.defaultFields as Record<string, unknown>;
          const phoneNumbers = (defaultFields?.phoneNumbers as Array<Record<string, unknown>>)?.[0];

          const { error } = await supabase.from("admin_quo_contacts").upsert({
            workspace_id: workspaceId,
            external_id: contact.id,
            first_name: defaultFields?.firstName,
            last_name: defaultFields?.lastName,
            emails: defaultFields?.emails,
            primary_phone: phoneNumbers?.value,
            company: defaultFields?.company,
            role: defaultFields?.role,
            custom_fields: contact.customFields || {},
            metadata: { external_contact_id: contact.id },
            created_at: new Date().toISOString(),
            updated_at: new Date().toISOString(),
          });

          if (error) {
            console.error(`Error upserting contact ${contact.id}:`, error);
            errors++;
          } else {
            synced++;
          }
        } catch (err) {
          console.error(`Error processing contact ${contact.id}:`, err);
          errors++;
        }
      }

      after = data.pageInfo?.endCursor;
      hasMore = !!after && contacts.length > 0;
    }

    console.log(`Synced ${synced} contacts for workspace ${workspaceId}, ${errors} errors`);
    return { synced, errors };
  } catch (err) {
    console.error(`Error syncing contacts for workspace ${workspaceId}:`, err);
    throw err;
  }
}

async function getWorkspaceIds(): Promise<string[]> {
  try {
    const { data, error } = await supabase
      .from("admin_workspaces")
      .select("id")
      .eq("is_active", true);

    if (error) {
      console.error("Error fetching workspaces:", error);
      return [];
    }

    return (data || []).map((w: Record<string, unknown>) => w.id as string);
  } catch (err) {
    console.error("Error getting workspace IDs:", err);
    return [];
  }
}

Deno.serve(async (req: Request) => {
  if (req.method === "OPTIONS") {
    return new Response(null, { headers: { "Access-Control-Allow-Origin": "*" } });
  }

  if (req.method !== "POST") {
    return new Response(JSON.stringify({ error: "Method not allowed" }), {
      status: 405,
      headers: { "Content-Type": "application/json" },
    });
  }

  try {
    if (!QUO_API_KEY) {
      return new Response(JSON.stringify({ error: "not_configured" }), {
        status: 503,
        headers: { "Content-Type": "application/json" },
      });
    }

    const workspaceIds = await getWorkspaceIds();
    const results = [];

    for (const workspaceId of workspaceIds) {
      const result = await syncContactsForWorkspace(workspaceId);
      results.push({ workspaceId, ...result });
    }

    return new Response(JSON.stringify({ success: true, results }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  } catch (err) {
    console.error("Sync error:", err);
    return new Response(JSON.stringify({ error: String(err) }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
});
